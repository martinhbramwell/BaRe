...
# echo -e "
#            ***  CURTAILED in 'repackageWithCorrectedSiteName()' ***
#            ========================================================
...
# echo -e "
#            ***  CURTAILED  ***
#         Inside restoreSocialLoginConfig()
#     ==========================================
# "${ENVIRONMENT_VARIABLES}"
# ";
# exit;
  ...
  if [ $? != 0 ]; then
    ...
    if [[ ! -f ${SOCIAL_LOGIN_CONF} ]]; then
      ...
      # echo -e "  - getting previous Social Login config...";
      # curl ${SWITCHES} GET --header "${AUTH_HEADER}" --header "${CONTENT_HEADER}" "${RESOURCE_URL}/${SLK}/google" | jq -r .
...
# exit;
  ...
  pushd ${TARGET_BENCH} >/dev/null;
    if [[ "X${BACKUP_FILE_DATE}X" != "XX" ]]; then
      ...
      # echo -e "bench --site ${ERPNEXT_SITE_URL} --force restore ${PASS} ${FILE} ${PRIV} ${DATA}";
      # exit;
...
if [[ ${SCRIPT_NAME} = ${THIS_SCRIPT} ]] ; then
  ...
  if [[ 1 == 1 ]]; then
    echo -e "\n      - Restoring database and files${pFAINT_BLUE}";
    restoreDatabase ${1};
