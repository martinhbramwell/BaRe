...
pushd ${TARGET_BENCH} >/dev/null;
  declare SOURCE_HOST=${TARGET_HOST};
  declare PRIVATE_FILES="sites/${SOURCE_HOST}/private/files";
  ...
  ERP_VERS=$(bench version | grep erpnext);
  ERPNEXT_VERSION=${ERP_VERS};
  echo -e "${ERPNEXT_VERSION}" > ${PRIVATE_FILES}/erpnextVersion.txt;
...
# cat ${PRIVATE_FILES}/erpnextVersion.txt;
# exit;
